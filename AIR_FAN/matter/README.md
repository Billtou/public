...
7. [Web UI:工程模式(OEM 白標)](#7-web-ui工程模式oem-白標)
   - 7.1 [進入工程模式](#71-進入工程模式)
   - 7.2 [使用說明連結](#72-使用說明連結)
   - 7.3 [品牌 Logo](#73-品牌-logo)
   - 7.4 [廠商名](#74-廠商名)
...
## 7. Web UI:工程模式(OEM 白標)

> ⚠️ 此區為 OEM 廠商品牌客製化用,**改廠商名會清除 Matter 配對並重啟**。一般使用者不需碰。

### 7.1 進入工程模式

底部 nav bar → **工程模式** → 輸入帳號密碼解鎖(此分頁開啟期間有效,離開頁面自動鎖回)。

預設帳密由廠商交付給經銷商,僅防誤觸,非安全邊界。

### 7.2 使用說明連結

> 即時生效,不需重啟、不需重新配對

修改首頁那條「使用說明 →」連結指向的網址。空白送出 = 還原預設(本 README)。

### 7.3 品牌 Logo

> 即時生效

替換 Web UI 所有頁面上方的 logo:
- 輸入 SVG URL(必須 `.svg` 結尾)→ 系統自動下載 + 瘦身(省 20–30%)+ 快取
- 瘦身後 ≤ 4 KB,建議用透明背景
- 「還原 AUTOMATE」按鈕可隨時回到內建 logo

### 7.4 廠商名

> **需重啟 + 需重新配對**

替換 Matter 在 Apple Home / HA 看到的廠牌名稱、hostname 前綴。

- 預設廠商名 `AUTOMATE` → hostname `automate-tuyafan-XXXXXX`
- 改為 `SmartHome` → hostname 變 `smarthome-tuyafan-XXXXXX`
- 二維碼 / 11 碼配對碼**不變**

按下「儲存新廠商配置並重啟配對」後裝置會:
1. 寫入新廠商名到 NVS
2. 清 Matter 配對(等同工廠重置 Matter 部分)
3. 重啟,進入配對模式

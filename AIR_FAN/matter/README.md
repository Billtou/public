...
<tr><td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">💡 燈光開關</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">獨立 OnOff endpoint,生態 App 看到一台裝置 + 一顆燈,對映 DP15</p>
</td>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🌐 內建 Web UI</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">區網直接連、免註冊,風扇 + 燈一頁掌控</p>
</td></tr>
</table>
...
<td>配對完成 → 出現「<b>TuyaFan-XXXXXX</b>」(可在 App 內改名),包含**風扇**與**燈光**兩個 icon</td></tr></table>
</td></tr>
</table>
...
4. App 自動完成,出現 `fan.tuyafan_xxxxxx` + `light.tuyafan_xxxxxx`(可在前端改名)
...
| **燈光 toggle** | 即時切換 OEM 燈光開關(DP15);Apple Home / HA 同步看到狀態變化 |
...
- ⚠️ **AC 110V/220V 高壓側請務必由合格電工安裝**,避免觸電
- 🔌 接線 / 拆卸**先斷電**,確認符合機型
...
| Matter Device Type | endpoint 1 = `0x002B` Fan(MultiSpeed + Rocking)<br>endpoint 2 = `0x0100` On/Off Light |
| MCU 介面 | UART 9600 8N1(原 Tuya DP 協定:DP1/DP3/DP5/DP15)|
| 風速段數(物理)| 7 段 |
| 風速映射(Matter)| 0–100 %(自動量化)|
| 擺頭(yaw)| 支援(對映 Matter `RockSetting` bit0)|
| 燈光開關 | 支援(獨立 endpoint,對映 DP15)|
| 待機功耗(模組)| < 0.3 W |
| 工作溫度 | -10 °C – 50 °C |
| 工作濕度 | 10% – 90% RH(無凝結)|
...
<p style="margin:0;font-size:14px"><b style="color:#1c3d5a">AUTOMATE</b> · AirFan · 韌體 v0.1.5</p>

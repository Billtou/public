...
* 第一步接入WIFI: 上電後上電靜待超過2分鐘，手機搜尋設備發出的AP熱點(autospace-pro 或 autospace-lite) 點選接入這個名稱的wifi後，手機切換到瀏覽器輸入192.168.4.1 即可填入自家wifi與密碼完成接入。

* 第二步調整參數: 手機掃描設備後蓋的二維條碼(QR Code)會跳轉到設置頁面，依序找到最合適現場環境的參數即可。也可以透過家裡的路由器得知被分配到的IP，直接在瀏覽器輸入IP也可以進入設置頁面。

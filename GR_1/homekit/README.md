...
<h1 style="font-size:42px;font-weight:800;margin:24px 0 6px 0;letter-spacing:-0.025em;color:#1c3d5a">Garage</h1>
<p style="font-size:17px;color:#5a6a7a;margin:0;font-weight:500">智能車庫門控制器 · HomeKit over Wi-Fi</p>
...
Garage 是 AUTOMATE 推出的 HomeKit 智能車庫門控制器,**直接接入 Apple Home**,不需專屬 Hub。

接到車庫門馬達的「開門 / 關門」乾接點上,**保留原廠遙控**,額外提供 Apple Home 開關 + 自動化能力。
...
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">磚塊一鍵開 / 關 · iPhone 全家共享</p>
</td>
<td width="50%" style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">📡 原廠遙控保留</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">RF 433MHz 遙控不受影響,Matter / HomeKit 控制並行</p>
</td>
</tr>
<tr>
...
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🔔 開門通知</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">Apple Home 推播,即時知道誰開了門</p>
</td>
</tr>
</table>
...
| 預設 hostname | `automate-garage-XXXXXX.local` |
...
| HomeKit 未配對 | **雙閃** 2 秒內閃兩下 | Wi-Fi 通了但 HomeKit 還沒加入 |
| 正常運轉 | **熄滅** | 正常狀態 |
...
<ul style="margin:10px 0 0;color:#1a2533">
<li>模組已正確接到車庫門控制板</li>
<li>路由器 <b>Wi-Fi 2.4 GHz</b> 訊號可達車庫</li>
...
<td>配對完成 → 出現「<b>Garage-XXXXXX</b>」,以**車庫門 icon** 顯示</td></tr></table>
</td></tr>
</table>
...
| 元件 | 功能 |
|---|---|
| 磚塊點擊 | 觸發 relay(切換開 / 關門)|
| 詳細頁 | 顯示當前狀態:**已開 / 開門中 / 已關 / 關門中 / 卡住** |
| Siri | 「嘿 Siri,開車庫門」 |
...
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🔔 門打開 → HomePod 提示音</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">家中誰開門即時提醒</p>
</td>
</tr>
</table>

<blockquote style="border-left:4px solid #ff6f48;background:#fff7f4;padding:14px 18px;margin:16px 0;border-radius:0 12px 12px 0;color:#5a3520">
⚠️ <b>安全提醒</b>:車庫門自動化涉及大門開關,Apple HomeKit 規定這類動作必須有「Home Hub 在家偵測」雙重確認。地理柵欄 + 時間條件建議二擇一,避免誤觸。
</blockquote>
...
| HA > 設定 > 人員 > 使用者 > 增加使用者 > 名稱填入 mqtt-user > 密碼填入 mqtt-pass|
### 6.2 設定步驟
...
|---|---|
| **韌體版本** | 例 `v0.0.2` |
| **可用記憶體** | 例 `220kB`(8MB flash 較大)|
| **運行時間** | 例 `2d 5h 30m` |
| **通訊狀態** | `HomeKit 已配對 / 正常 (wifi)` |
| **製造日期** | `YYYY-MM`,出廠燒入 |
...
| 欄位 | 顯示 |
|---|---|
| 門狀態 | 已關 / 開門中 / 已開 / 關門中 / 卡住 |
| 上次觸發 | 時間戳 + 來源(Apple Home / 遙控 / 物理按鈕)|
| Relay 計數 | 累計觸發次數(維修參考)|
...
<blockquote style="border-left:4px solid #ff6f48;background:#fff7f4;padding:14px 18px;margin:16px 0;border-radius:0 12px 12px 0;color:#5a3520">
⚠️ 若沒接限位開關,Apple Home 只能顯示「最後一次觸發 = 開 / 關」(toggle 估計),無法真實知道門狀態。
</blockquote>
...
整個過程約 1–2 分鐘,期間 HomeKit 短暫離線(車庫門馬達不受影響,原廠遙控仍可用)。
...
💡 <b>HomeKit-only reset</b> — 只清配對 + Wi-Fi。設備認證 / 防複製綁定保留。
</blockquote>
...
| 現象 | 原因 | 處理 |
|---|---|---|
| Apple Home 點開關但門沒動 | Relay 接線錯 / 控制板無 PB 端子 | 檢查接線,試手動觸發 relay |
| 門狀態永遠是「已關」 | 未接限位開關 | 接乾簧管 + 磁鐵 |
...
- ⚠️ **車庫門馬達 AC 高壓側請務必由合格電工安裝**,觸電會致命
- 🚪 **絕對不要**在車庫門下方人 / 車正在通過時觸發開關
- 🔌 接線 / 拆卸**先斷電**
...
| 項目 | 規格 |
|---|---|
| 產品名稱 | Garage HomeKit Controller |
| 品牌 | AUTOMATE |
| 通訊協定 | HomeKit (HAP) over Wi-Fi |
...
<p style="margin:0;font-size:14px"><b style="color:#1c3d5a">AUTOMATE</b> · Garage · 韌體 v0.0.2</p>

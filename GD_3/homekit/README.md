<!-- GD-3 README — Argon-style (Automate brand) -->
<!-- HomeKit-direct rolling-door opener (ESP32-S3 + W5500 Ethernet). NO multi-ecosystem section. -->
...
<h1 style="font-size:42px;font-weight:800;margin:24px 0 6px 0;letter-spacing:-0.025em;color:#1c3d5a">GD-3</h1>
<p style="font-size:17px;color:#5a6a7a;margin:0;font-weight:500">智能鐵卷門控制器 · HomeKit over Ethernet / Wi-Fi</p>
...
GD-3 是 AUTOMATE 推出的 HomeKit 智能鐵卷門控制器,**直接接入 Apple Home**,不需專屬 Hub。

接到鐵卷門馬達的「開 / 停 / 關」三組乾接點上,**保留原廠遙控**,額外提供 Apple Home 開關 + 自動化能力 + 雙雷射光柵防夾安全機制。
...
<p style="margin:0;font-weight:700;color:#1c3d5a">🏠 Apple Home 開關門</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">磚塊一鍵開 / 停 / 關 · iPhone 全家共享</p>
</td>
<td width="50%" style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">📡 原廠遙控保留</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">RF 433MHz 遙控不受影響,HomeKit 控制並行</p>
</td>
</tr>
<tr>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🛡️ 雙雷射光柵防夾</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">關門中偵測障礙物 → 自動反向開啟</p>
</td>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🔌 有線優先 + WiFi 備援</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">插網線走 Ethernet,沒線自動切 Wi-Fi</p>
</td>
</tr>
<tr>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">📍 地理柵欄自動化</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">家人靠近 → 自動開門</p>
</td>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🔔 障礙物通知</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">雷射偵測 → Apple Home 顯示警示</p>
</td>
</tr>
</table>
...
| 項目 | 內容 |
|---|---|
| 通訊協定 | HomeKit (HAP) over Ethernet / Wi-Fi |
| 網路 | W5500 SPI Ethernet 10/100 Mbps(RJ45)+ Wi-Fi 2.4 GHz 802.11 b/g/n |
| 處理器 | ESP32-S3(Xtensa LX7 dual-core, 240 MHz, 4 MB Flash)|
| 控制接口 | 3 路繼電器(UP / DOWN / STOP)+ 磁簧門位 + 雙雷射光柵 + 3 實體按鍵 |
| Apple Home Service | GarageDoorOpener + Pause Switch(linked)|
| 安全 | 雙雷射光柵防夾(關門中偵測障礙物自動反向開啟)|
...
<li><b>DC IN</b>:5–24V DC(由鐵卷門控制板供應,或用 USB 5V)</li>
<li><b>RELAY UP / DOWN / STOP</b>:3 路乾接點(NO),分別接馬達的開 / 關 / 停按鈕迴路</li>
<li><b>SENSOR(磁簧)</b>:門關到底的下極限訊號(門框 + 門板各一塊磁鐵)</li>
<li><b>LASER 1 / LASER 2</b>:雙雷射光柵,跨門口安裝,關門中偵測到障礙物自動反向開啟</li>
<li><b>BTN UP / DOWN / STOP</b>:3 顆面板實體按鍵</li>
<li><b>RJ45</b>:Ethernet(W5500),有網線優先,沒網線自動 fallback Wi-Fi</li>
...
| Model | `Garage-GD3` |
| 預設裝置名稱 | `GD3-XXXXXX`(後 6 碼為 Wi-Fi MAC)|
| 預設 hostname | `automate-gd3-XXXXXX.local` |
...
| 條件 | LED | 意義 |
|---|---|---|
| 重置警告(長按 ≥ 5s)| **急閃** 100ms 亮 / 100ms 滅 | 「再按 5 秒會觸發工廠重置」 |
| Wi-Fi / Eth 未連線 | **慢閃** 0.2s 亮 / 1.8s 滅 | 配網模式 / Wi-Fi 重連 |
| HomeKit 未配對 | **雙閃** 2 秒內閃兩下 | 網路通了但 HomeKit 還沒加入 |
| 正常運轉 | **熄滅** | 正常狀態 |
...
<ul style="margin:10px 0 0;color:#1a2533">
<li>模組已正確接到鐵卷門控制板</li>
<li>網線已插好(若用 Ethernet),或路由器 <b>Wi-Fi 2.4 GHz</b> 訊號可達</li>
<li>iPhone <b>已加入「家庭」App</b>(iOS 16.1+)</li>
<li>家中有 <b>HomeKit 家居中樞</b>:HomePod / Apple TV / iPad</li>
<li>手機跟模組距離 &lt; 1 公尺</li>
</ul>
</div>
...
<td>若無網線:手機連到 SoftAP「<b>Garage-GD3-XXXXXX</b>」(開放網路)→ 開 <code>http://192.168.4.1/</code> → 填家裡 Wi-Fi 帳密 → 自動重啟。<br>(有插網線可跳過這步)</td></tr></table>
</td></tr>
<tr><td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)" valign="top">
...
<td>對準 QR Code 掃描;或輸入 8 碼 setup code</td></tr></table>
</td></tr>
<tr><td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)" valign="top">
...
<td>配對完成 → 出現「<b>GD3-XXXXXX</b>」,以**車庫門 icon** 顯示</td></tr></table>
</td></tr>
</table>
...
<p style="margin:0;font-weight:700;color:#1c3d5a">📍 我到家 → 自動開門</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">地理柵欄,iPhone 進入範圍觸發</p>
</td>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🌙 22:00 門還開著 → 推播提醒</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">忘記關門安全提醒</p>
</td>
</tr>
<tr>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🏠 大家都離家 → 自動關門</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">最後一個離開檢查</p>
</td>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🚨 雷射偵測障礙 → HomePod 警示</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">設備自動 STOP 反向後額外通知</p>
</td>
</tr>
</table>
...
| **可用記憶體** | 例 `200kB` |
| **運行時間** | 例 `2d 5h 30m` |
| **通訊狀態** | `HomeKit 已配對 / 正常 (eth)` 或 `(wifi)` |
| **製造日期** | `YYYY-MM`,出廠燒入 |
...
1. 確認控制板有「**遠端按鈕**」端子(通常標記為 UP / STOP / DOWN 或 OPEN / STOP / CLOSE)
2. 將模組對應端子接過去(UP↔開、DOWN↔關、STOP↔停),全部是乾接點(NO)
3. 模組 **DC IN** 可從控制板上的 12V/24V 取電

**接磁簧門位 sensor**:

1. 在門框 + 門板上各裝**乾簧管 + 磁鐵**,門完全關時兩塊靠近導通
2. 接到 **SENSOR + GND**
3. Apple Home 才能正確顯示「已關閉」狀態

**接雙雷射光柵**:

1. 兩組光柵跨門口安裝(發射 + 接收一組,共兩組)
2. 安裝高度建議 30~50 cm(同時防兒童與車輛被夾)
3. 接到 **LASER1 + LASER2 + GND**
4. 雷射電源由模組自動 gating — 只在關門中通電,省電 + 延長雷射壽命

**接網路**:

- **首選**:RJ45 接 Ethernet(W5500)— 訊號最穩
- **備援**:沒接網線時自動走 Wi-Fi 2.4 GHz
- **配網**:Wi-Fi 沒設定時開 SoftAP「Garage-GD3-XXXXXX」captive portal
...
2. 系統自動檢查最新版本
3. 點擊「**開始更新**」→ 確認 → 自動下載
4. 下載完成自動重啟,Web UI reload 回首頁

整個過程約 1–2 分鐘,期間 HomeKit 短暫離線(鐵卷門馬達不受影響,原廠遙控仍可用)。
...
<tr><td style="padding:10px 16px;border-top:1px solid #eef1f6">行程 / 防抖等 timing 設定</td><td align="center" style="padding:10px 16px;border-top:1px solid #eef1f6;color:#27ae60">✅ 保留</td></tr>
...
💡 <b>HomeKit-only reset</b> — 只清配對 + Wi-Fi。設備認證 / 防複製綁定 / 出廠製造日期保留。
</blockquote>
...
| 現象 | 原因 | 處理 |
|---|---|---|
| 家庭 App 找不到裝置 | 距離太遠 / Wi-Fi 是 5 GHz | 距離 < 1 m、走 2.4 GHz |
...
- ⚠️ **鐵卷門馬達 AC 高壓側請務必由合格電工安裝**,觸電會致命
- 🚪 **絕對不要**在鐵卷門下方人 / 車正在通過時觸發開關
- 🛡️ **雷射光柵是被動安全機制**,不取代正常注意義務;故障時門依舊會關
- 🔌 接線 / 拆卸**先斷電**
- 📍 自動化(尤其地理柵欄)務必雙重條件,避免誤觸傷人
- 🌡 工作環境溫度 -10–60 °C(車庫高溫環境也耐)
- 🌧 模組安裝請避雨(室內安裝,非戶外)
- 📻 模組內含無線電路,改裝會違反電信法規
...
| 項目 | 規格 |
|---|---|
| 產品名稱 | GD-3 HomeKit Controller |
| 品牌 | AUTOMATE |
| 通訊協定 | HomeKit (HAP) over Ethernet / Wi-Fi |
| Ethernet | W5500 10/100 Mbps(RJ45)|
| Wi-Fi | 2.4 GHz 802.11 b/g/n |
| 處理器 | ESP32-S3(Xtensa LX7 dual-core, 240 MHz)|
| Flash | 4 MB |
| Apple Home Service | GarageDoorOpener + Pause Switch |
| DC 輸入 | 5–24V DC |
| Relay 輸出 | 3× NO 乾接點(UP / DOWN / STOP),5A 最大負載 |
| 感測輸入 | 1× 磁簧(下極限)+ 2× 雷射光柵(安全)|
| 實體按鍵 | 3× 方向(UP / STOP / DOWN)+ 1× 多功能 |
| 待機功耗(模組)| < 0.4 W |
| 工作溫度 | -10 °C – 60 °C |
| 工作濕度 | 10% – 90% RH(無凝結)|

<!-- AirOne-AC README — Argon-style (Automate brand) -->
<!-- HomeKit-direct AC controller. NO multi-ecosystem section. -->
...
<h1 style="font-size:42px;font-weight:800;margin:24px 0 6px 0;letter-spacing:-0.025em;color:#1c3d5a">AirOne-AC</h1>
<p style="font-size:17px;color:#5a6a7a;margin:0;font-weight:500">智能冷氣控制器 · HomeKit over Wi-Fi · 多廠牌相容</p>
...
AirOne-AC 是 AUTOMATE 推出的 HomeKit 智能冷氣控制器,**直接接入 Apple Home**,不需專屬 Hub 或雲端帳號。

支援多家原廠冷氣機,透過 UART 串接原廠通訊協定:
...
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">遙控器走原 IR 通路,跟 HomeKit 控制並行</p>
</td>
</tr>
<tr>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
<p style="margin:0;font-weight:700;color:#1c3d5a">🌡️ 目標溫度 16–30 °C</p>
<p style="margin:6px 0 0;font-size:14px;color:#5a6a7a">Apple Home 溫度滑桿即時同步</p>
</td>
<td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
...
| 預設 hostname | `automate-airone-XXXXXX.local` |
...
| Wi-Fi 未連線 | **慢閃** 0.2s 亮 / 1.8s 滅 | 配網模式 / Wi-Fi 重連 |
...
走 IR / RF 通路,**獨立於模組**:
- 遙控控制觸發的狀態變化,Apple Home 端 1 秒內同步看到
- Apple Home 控制 → 原廠面板 LED / LCD 也會同步
...
<li>路由器 <b>Wi-Fi 2.4 GHz</b> 可用</li>
<li>iPhone / iPad <b>已加入「家庭」App</b>(iOS 16.1+)</li>
<li>家中至少有一台 <b>HomeKit 家居中樞</b>:HomePod / HomePod mini / Apple TV(4K)/ iPad</li>
<li>手機跟冷氣距離 &lt; 1 公尺</li>
</ul>
</div>
...
<td>通電冷氣,等模組完成上電(約 5 秒,LED 慢閃配對模式)</td></tr></table>
</td></tr>
<tr><td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)" valign="top">
...
<td>對準 QR Code 掃描;或選「沒有代碼」→ 輸入 8 碼 setup code</td></tr></table>
</td></tr>
<tr><td style="background:#fff;padding:18px;border-radius:14px;border:1px solid #e5e7eb;box-shadow:0 2px 8px rgba(0,0,0,0.04)" valign="top">
...
<td>配對完成 → 出現「<b>AirOne-XXXXXX</b>」,以**冷氣 icon** 顯示</td></tr></table>
</td></tr>
</table>
...
<p style="margin:0;font-weight:700;color:#1c3d5a">🌡️ 室溫 > 28 °C → 開冷氣</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">回家前自動降溫</p>
</td>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🌙 22:00 → 切到暖氣 26 °C</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">睡眠模式</p>
</td>
</tr>
<tr>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🏠 離家 → 自動關機</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">節能</p>
</td>
<td style="background:#fff;padding:14px;border-radius:12px;border:1px solid #e5e7eb">
<p style="margin:0;font-weight:700;color:#1c3d5a">🌧️ 偵測下雨 → 切除濕模式</p>
<p style="margin:6px 0 0;font-size:13px;color:#5a6a7a">搭配天氣 App</p>
</td>
</tr>
</table>
...
- **主機名稱**:`http://automate-airone-XXXXXX.local`
...
|---|---|
| **大溫度數字** | 即時目前室內溫度 |
| **目標溫度滑桿** | 拖動 = 設目標(16–30 °C),即時送 UART |
| **模式 grid** | 自動 / 冷氣 / 暖氣 / 除濕 / 送風(點切換) |
| **風量 segment** | 自動 / 弱 / 中 / 強 |
| **電源 toggle** | 即時切換電源 ON / OFF |
| **擺風 toggle** | 即時切換左右擺風(看機型支援)|

---

<h2 style="color:#1c3d5a;border-bottom:3px solid #ff6f48;padding-bottom:8px;margin-top:48px;font-size:28px">7. 模式與風量</h2>
...
4 段:**自動 / 弱 / 中 / 強**(部分機型有「靜音」)。Apple Home 滑桿任意中間值會自動量化。

---

<h2 style="color:#1c3d5a;border-bottom:3px solid #ff6f48;padding-bottom:8px;margin-top:48px;font-size:28px">8. 防霉守護(MoldGuard)</h2>
...
<tr><td style="padding:10px 16px;border-top:1px solid #eef1f6">冷氣 Profile(廠牌協定)</td><td align="center" style="padding:10px 16px;border-top:1px solid #eef1f6;color:#27ae60">✅ 保留</td></tr>
<tr><td style="padding:10px 16px;border-top:1px solid #eef1f6">MoldGuard 設定</td><td align="center" style="padding:10px 16px;border-top:1px solid #eef1f6;color:#27ae60">✅ 保留</td></tr>
...
<blockquote style="border-left:4px solid #007AFF;background:#eff7ff;padding:14px 18px;margin:16px 0;border-radius:0 12px 12px 0;color:#1c3d5a">
💡 <b>HomeKit-only reset</b> — 只清配對 + Wi-Fi。冷氣 Profile / MoldGuard 設定保留。
</blockquote>
...
| 現象 | 原因 | 處理 |
|---|---|---|
| Apple Home 拉滑桿但冷氣沒動 | UART 通訊異常 / 機台關機 | 確認冷氣電源 ON;重啟模組 |
| 原廠遙控可動,Apple Home 沒同步 | UART 通訊異常 | 重啟模組 / 等 1 秒 polling 同步 |
...
| 項目 | 規格 |
|---|---|
| 產品名稱 | AirOne-AC HomeKit Controller |
| 品牌 | AUTOMATE |
| 通訊協定 | HomeKit (HAP) over Wi-Fi |
...
<p style="margin:0;font-size:14px"><b style="color:#1c3d5a">AUTOMATE</b> · AirOne-AC · 韌體 v0.2.6</p>

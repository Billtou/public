...
MODEL_INIT_SEQUENCES = {
    "PANLEE": "esphome::gc9503v::INITCMD_PANLEE",
    "VIEWE":  "esphome::gc9503v::INITCMD_VIEWE",
}

async def to_code(config):
    var = cg.new_Pvariable(config[CONF_ID])
    await display.register_display(var, config)
    model_key = next(k for k, v in MODELS.items() if v == config[CONF_MODEL])
    cg.add(var.set_init_sequence(cg.RawExpression(MODEL_INIT_SEQUENCES[model_key])))

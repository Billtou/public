...
9. [Web UI:工程模式(後期維修用)](#9-web-ui工程模式後期維修用)

10. [睡眠模式自動化(進階)](#10-睡眠模式自動化進階)

11. [甲醛在 Apple Home 與 Home Assistant 的限制](#11-甲醛在-apple-home-與-home-assistant-的限制)
...
AirCube 主板支援兩種 LED 配置,在 Web UI 工程模式切換(章節 9):
...
5. 配對完成 → 會出現「**空氣品質感測器**」配件
   ...
   - 溫度 / 濕度為獨立配件
   - 詳見[章節 11](#11-甲醛在-apple-home-與-home-assistant-的限制) 關於甲醛在 Apple Home 的顯示限制
...
## 11. 甲醛在 Apple Home 與 Home Assistant 的限制

**Apple Home、Home Assistant 對 Matter AirQualitySensor 的支援不完整**

- ❌ **Formaldehyde / 甲醛**

AirCube 韌體確實有送出 CH2O 的 Matter cluster 資料,但目前 Matter 與智能生態對於轉換協議還沒確定,導致他們不認識甲醛。目前以螢幕顯示方式呈現。

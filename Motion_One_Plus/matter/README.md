...
- **雷達感測窗**:24 GHz 毫米波,**不可遮蓋**(膠帶 / 厚紙板會嚴重衰減訊號)
...
1. 通電,等 LED 慢閃
2. 開 SmartThings App → **+** → **裝置** → **掃描 Matter QR**
...
Matter 規範允許**同一台裝置同時加入多個生態**,最多 **5 個 Fabric**。例如同時被 Apple Home 與 Google Home 控制,動作偵測會同步出現在兩個 App。
...
2. 滑到底,找 **「轉給其他生態 / Turn On Pairing Mode」**
3. App 顯示一組 11 碼**新配對碼**(15 分鐘內有效)
4. 在第二個生態 App「加入配件」流程輸入這組新碼

### 6.3 從 Google Home 分享給其他生態

1. Google Home App → 點擊裝置 → 設定 → **Matter 裝置 ID**
2. **新增至其他平台 / Linked Matter apps & services**
3. 取得新配對碼
4. 在第二個生態 App 輸入此碼

### 6.4 從 SmartThings 分享給其他生態

1. SmartThings App → 裝置 → 設定 → **Share with other ecosystems**
2. 取得新配對碼
3. 在第二個生態 App 輸入此碼
